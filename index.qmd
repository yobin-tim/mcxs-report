...
colnames(df)                   <- c("m1","cashrate","usffr")
df["cpi"]                      <- as.numeric(cpi.qtr[268:length(cpi.qtr)])                 
df["consumption"]              <- as.numeric(tail(consumption.qtr,nrow(df)))
df["gdp"]                      <- as.numeric(tail(gdp.qtr,nrow(df)))
df["homeloan"]                 <- as.numeric(tail(homeloan.qtr,nrow(df)+1))[-133]
df["nhomes"]                   <- as.numeric(tail(nhomes.qtr,nrow(df)+1))[-133]
df                             <- head(df,-4) # drop the last 4 obs 
df["homeprice"]                <- NA 
df[55:128,"homeprice"]         <- as.numeric(homeprice.qtr)
df                             <- na.omit(df)
```
