...
rm(list = ls())
pkgs <- c('ggplot2', 'tidyr', 'dplyr', 'lubridate', 'readrba', 'readabs', 'tidyverse', "xts", "plotly", "rmarkdown", "zoo", "quantmod", "HDInterval", "plot3D", "mvtnorm")
...
df                                      <- df %>% select("Nominal GDP (log)", "Aggregate Consumption (log)",
                                                         "% Change in CPI, yoy", "US FFR Max Target Rate", "Quantity of new homes",
                                                         "House Price Index", "Owner-occupied home loan rates",
                                                         "M1 Money Supply (logs)", ,
...
```{r : Plotting the effects of the AU CR shock}
...
for (n in 1:N){
  ...
  # if (n==5 | n==6){
    axis(1,c(1,2,5,7,9),c("","1 quarter","1 year","6 quarters", "2 years"))
  # } else {
  #   axis(1,c(1,2,5,7),c("","","",""))
  # }
...
#| fig-cap-location: top
cashrate.irf = plot_irf(9, data.irf_fevd, parameters)
```
...
#| fig-cap-location: top
usffr.irf = plot_irf(4, data.irf_fevd, parameters)
```
...
```{r}
variable.choice = "House Price Index"
position = which(colnames(df) == variable.choice)
